...
import { ForeachPrototypeParser, generateForeachMarkers, parseForeachMarker } from './foreach-comment-markers';
...
function testMarkerParsing(): void {
  ...
  if (startResult?.type === 'foreach-start' && 
...
function testHTMLTransformation(): void {
  ...
  const processed = parser.processForeachMarkers(transformed, { '100': ['Apple', 'Banana', 'Cherry'] });
...
function testCSSCompatibility(): void {
  ...
  currentDOM.forEach((node, i) => {
    ...
    const isLastParagraph = node.type === 'p' && 
      currentDOM.slice(i + 1).every(n => n.type !== 'p');
    console.log(`  ${node.type}: "${node.content}" (last-child: ${isLastChild}, last p: ${isLastParagraph})`);
  });
  ...
  proposedDOM.forEach((node, i) => {
    ...
    const isLastParagraph = node.type === 'p' && 
      proposedDOM.slice(i + 1).every(n => n.type !== 'p');
    console.log(`  ${node.type}: "${node.content}" (last-child: ${isLastChild}, last p: ${isLastParagraph})`);
  });
...
function demonstrateImplementationPlan(): void {
  ...
  console.log('   - Replace ServerTemplateElement creation with comment insertion');
  ...
  console.log('   - Clone content between markers instead of template children\n');
