...
import {
  RT_ATTR_VALUE_PREFIX,
  RT_CLASS_VALUE_PREFIX,
  RT_EVENT_VALUE_PREFIX,
  RT_STYLE_VALUE_PREFIX,
  RT_TEXT_VALUE_PREFIX,
} from '../runtime/web/web-scope';
...
export function load(source: Source): CompilerScope {
  ...
  const wrapSpecialTags = (e: ServerElement) => {
    ...
    if (e.tagName === k.DEFINE_DIRECTIVE) {
      ...
      if (e.getAttributeNode(k.NAME_ATTR)) {
        error(
          tagAttr.valueLoc!,
          `attribute "${k.NAME_ATTR}" now allowed in definitions`
        );
        return { type, wrapper: null };
      }
  ...
  const load = (e: ServerElement, p: CompilerScope) => {
    ...
    if (needsScope(e)) {
      const { type, wrapper, definesTag, extendsTag, extendsAttr } =
        wrapSpecialTags(e);
      if (wrapper) {
      ...
      if (definesTag) {
        ...
        if (scope.xtends && typeof scope.xtends !== 'string') {
          error(
            extendsAttr?.valueLoc!,
            `cannot extend another definition ("${extendsTag}")`
          );
        }
        definition = scope;
      }
      ...
      if (
        ...
        texts.forEach((text, index) => {
          ...
          const nextSibling =
            childIndex + 1 < p.childNodes.length
              ? p.childNodes[childIndex + 1]
              : null;
          if (nextSibling) {
...
function needsScope(e: dom.Element): boolean {
  ...
  if (ATOMIC_TEXT_TAGS.has(e.tagName)) {
    const t =
      e.childNodes.length === 1 ? (e.childNodes[0] as ServerText) : null;
    if (
      t?.nodeType === dom.NodeType.TEXT &&
      typeof t.textContent === 'object'
    ) {
      return true;
    }
  }
...
function addSlotMap(
  definition: CompilerScope,
  template: dom.TemplateElement,
  source: Source
) {
