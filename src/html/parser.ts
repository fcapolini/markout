...
/**
 * Thrown to unwind the stack when an irrecoverable error is found. The error
 * itself is collected in `Source.errors` before throwing, so this carries no
 * message; anything else escaping the parser is a bug and must not be
...
export function parse(
  ...
  try {
    ...
    // nop: the error was already collected in ret.errors
  }
...
function parseNodes(
  ...
  while ((i2 = s.indexOf('<', i1)) >= 0) {
    ...
    if ((i2 = skipName(src, i1)) > i1) {
      ...
      i3 = i1;
    } else if (!closure && (i2 = skipComment(src, i1, errors)) > i1) {
      if (i4 > i3) {
        parseText(p, src, i3, i4, errors);
      }
      const a = i1 + 3;
  ...
  while ((i2 = skipName(src, i1, true)) > i1) {
    ...
    if (s.charCodeAt(i1) === EQ) {
      ...
      if (a && (quote === QUOT || quote === APOS)) {
        ...
        i1 = parseValue(a, src, i1 + LEXP.length, quote, '}', errors);
      } else {
...
function parseQuotedValueExpression(
  ...
  literals.length === expressions.length &&
    literals.push({
      start: i2,
      end: i2,
      loc: src.loc(i2, i2),
  ...
  makeTextInterpolation(a, literals, expressions, src);
}

function makeTextInterpolation(
  a: dom.ServerAttribute,
  literals: {
    start: number;
    end: number;
  ...
  const exp: acorn.Expression = {
    type: 'TemplateLiteral',
    quasis: literals.map((lit, i) => ({
      type: 'TemplateElement',
      value: { raw: lit.value, cooked: lit.value },
      tail: i === literals.length - 1,
      start: lit.start,
      end: lit.end,
      loc: lit.loc,
    })),
    ...
    loc: src.loc(a.valueLoc!.i1, a.valueLoc!.i2),
  };
  a.value = exp;
}
...
function parseUnquotedExpressionValue(
  ...
  const s = src.s;
  const exp = parseExpression(src, i1, errors);
  let i2 = exp.end;
  i2 = skipBlanks(s, i2);
 ...
 * parser's.
 */
function skipName(src: Source, i: number, isAttribute = false) {
  const s = src.s;
  while (i < s.length) {
...
function skipComment(src: Source, i1: number, errors: PageError[]) {
  const s = src.s;
  if (
