...
describe('node', () => {
  it('should append an element', () => {
    ...
    doc.appendChild(new ServerElement(doc, 'html', LOC));
    assert.equal(doc.toString(), `<html></html>`);
  });

  it('should append and unlink a text node', () => {
    const doc = new ServerDocument('test');
    const root = doc.appendChild(
      new ServerElement(doc, 'html', LOC)
    ) as Element;
    // unlink() is server-side only, so this is typed as the implementation
    const text = root.appendChild(
      new ServerText(doc, 'test', LOC)
    ) as ServerText;
...
describe('attributes', () => {
  it('should overwrite an existing attribute', () => {
    ...
    e.setAttribute('a', '2');
    assert.equal(e.attributes.length, 1);
    assert.equal(e.getAttribute('a'), '2');
    assert.equal(e.toString(), `<html a="2"></html>`);
  });
  ...
  it('should remove an attribute', () => {
    ...
    e.setAttribute('b', '2');
    e.removeAttribute('a');
    assert.deepEqual(e.getAttributeNames(), ['b']);
    assert.equal(e.toString(), `<html b="2"></html>`);
  });

  it('should ignore removal of a missing attribute', () => {
    const doc = new ServerDocument('test');
    const e = new ServerElement(doc, 'html', LOC);
    e.setAttribute('a', '1');
    e.removeAttribute('nonexistent');
    assert.deepEqual(e.getAttributeNames(), ['a']);
  });
});
...
describe('document', () => {
  it('should create elements and text nodes', () => {
    const doc = new ServerDocument('test');
    const root = doc.appendChild(doc.createElement('html')) as Element;
    assert.equal((root as ServerElement).tagName, 'HTML');
    root.appendChild(doc.createTextNode('a & b'));
    assert.equal(doc.toString(), `<html>a & b</html>`);
  });
  ...
  it('should clone a document', () => {
    ...
    root.appendChild(new ServerText(doc, 'text', LOC));
    const clone = doc.clone(null, null);
    assert.notEqual(clone, doc);
    assert.equal(clone.toString(), doc.toString());
    assert.equal(clone.toString(), `<html>text</html>`);
  });
...
describe('fragment', () => {
  it('should implement firstElementChild property', () => {
    const doc = new ServerDocument('test');
    const frag = new ServerDocumentFragment(LOC);
    assert.isNull(frag.firstElementChild);
    frag.appendChild(new ServerText(doc, 'text', LOC));
    assert.isNull(frag.firstElementChild);
    const e = frag.appendChild(new ServerElement(doc, 'p', LOC));
    assert.equal(frag.firstElementChild, e);
    assert.equal(frag.toString(), `text<p></p>`);
  });
});
...
describe('classList', () => {
  ...
  it('should integrate className, classList and `class` attribute (1): via className', () => {
    const doc = new ServerDocument('test');
    const e = new ServerElement(doc, 'html', LOC);
  ...
  it('should integrate className, classList and `class` attribute (2): via setAttribute', () => {
    const doc = new ServerDocument('test');
    const e = new ServerElement(doc, 'html', LOC);
    assert.equal(e.toString(), '<html></html>');
    assert.notExists(e.getAttribute('class'));
...
describe('template', () => {
  ...
  it('should support cloning nested template content into a parent', () => {
    const doc = new ServerDocument('test');
    const root = doc.appendChild(
      new ServerElement(doc, 'html', LOC)
    ) as Element;
