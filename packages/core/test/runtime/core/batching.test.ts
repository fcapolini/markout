...
import { CoreValue } from '../../../src/runtime/core/core-value';
...
describe('DOM Update Batching', () => {
  ...
  it('eagerly queues a static value when setCB is called', () => {
    const value = new CoreValue({ val: 'static' }, scope);
    value.setCB(vi.fn());
    expect(context.pending.has(value)).toBe(true);
  });

  it('does not eagerly queue an exp-based value when setCB is called', () => {
    const value = new CoreValue({ exp: () => 'computed' }, scope);
    value.setCB(vi.fn());
    expect(context.pending.has(value)).toBe(false);
  });
  ...
  it('should batch multiple value updates in single reactive cycle', () => {
    ...
    // Create values
    const value1 = new CoreValue({ val: 'initial1' }, scope);
    const value2 = new CoreValue({ val: 'initial2' }, scope);
    const value3 = new CoreValue({ val: 'initial3' }, scope);
  ...
  it('should deduplicate multiple updates to same value', () => {
    ...
    const value = new CoreValue({ val: 'initial' }, scope);
  ...
  it('should handle reactive propagation with batching', () => {
    ...
    const callback = vi.fn();
    const value = new CoreValue({ val: 'test' }, scope);
    value.setCB(callback);
  ...
  it('should handle nested reactive cycles correctly', () => {
    ...
    const values = callbacks.map((cb, i) => {
      const val = new CoreValue({ val: i }, scope);
      val.setCB(cb);
      return val;
    });
  ...
  it('should batch updates across multiple scopes', () => {
    // Create child scope
    const childScope = new CoreScope(
      { id: 'child', values: {} },
      context,
      scope
    );
    ...
    const parentValue = new CoreValue({ val: 'parent' }, scope);
    const childValue = new CoreValue({ val: 'child' }, childScope);
