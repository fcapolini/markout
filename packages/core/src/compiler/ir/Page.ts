...
/**
 * A page's compiled props: JavaScript for the expressions, JSON for
 * everything else.
 */
export interface CompiledProps {
  /** the expressions, as a JavaScript array literal */
  exps: string;
...
export class Page {
  ...
  /**
   * The props stage7 generated, in the two halves a page carries them in:
   * the expressions as a JavaScript array literal, and the scope tree as
   * JSON referring to them by index.
   *
   * Two artifacts rather than one string, because the JSON is not
   * JavaScript and never becomes any: escaping it into a string literal
   * doubled every quote it held, which is bytes paid for nothing. It
   * travels in a `<script type="application/json">` of its own, where it is
   * text. See stage7's emitProps and render/props.ts's loadProps.
   *
   * This pair is what the SERVER runs.
   */
  props?: CompiledProps;
  /**
   * The same, with every `:server-` expression taken out: what the browser
   * is given, and what the two props `<script>`s carry.
   *
   * The same pair as `props` on a page that declares no server value, so
   * nothing pays for this feature without using it.
   */
  clientProps?: CompiledProps;
  /**
