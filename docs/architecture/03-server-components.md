...
C4Component
    ...
    Rel(browser, express_app, "requests")
    Rel(express_app, rate_limiter, "applies")
    Rel(express_app, compression, "applies")
    Rel(express_app, markout_middleware, "routes")
    Rel(express_app, static_server, "serves")
    Rel(express_app, logger, "logs")
    
    Rel(markout_middleware, compiler, "compiles")
    Rel(markout_middleware, runtime, "executes")
    Rel(markout_middleware, filesystem, "reads")
    
    Rel(pm2_manager, express_app, "manages")
    Rel(health_monitor, express_app, "monitors")
    Rel(exit_handler, express_app, "handles")
    
    Rel(logger, filesystem, "writes")
