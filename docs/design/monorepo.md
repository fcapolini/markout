...
| | Result |
| --- | --- |
| `"@markout-lang/core": "workspace:*"` | **rejected** — `EUNSUPPORTEDPROTOCOL` |
...
verbatim, so a consumer of the CLI would resolve `@markout-lang/core@*` to
...
| --- | --- | --- |
| base | [kits.ts](../../packages/core/src/kits.ts), [paths.ts](../../packages/core/src/paths.ts) | `@markout-lang/core` |
| html | [src/html/](../../packages/core/src/html/) | `@markout-lang/core` |
| publish | [publish.ts](../../packages/core/src/publish.ts) | `@markout-lang/core` |
| runtime | [src/runtime/](../../packages/core/src/runtime/) | `@markout-lang/core` |
| compiler | [src/compiler/](../../packages/core/src/compiler/) | `@markout-lang/core` |
| render | `render.ts`, `serialize.ts`, `runtime-bundle.ts` | `@markout-lang/core` |
| http | `middleware.ts`, `livereload.ts`, `watcher.ts`, `logger.ts` | `@markout-lang/express` |
| cli | `cli.ts`, `defaults.ts`, `index.ts`, `server/index.ts`, `build.ts`, `exit-hook.ts` | `@markout-lang/cli` |
...
- **`@markout-lang/core`** — compile and render, no HTTP. Dependencies: acorn,
  escodegen, estraverse, entities. This is what the extension imports.
- **`@markout-lang/express`** — the middleware and the machinery it drives: the
...
- **`markout`** — the bin, the `Server` class, `build`. Depends on both, plus
  ...
  **`Server` stays in the CLI rather than moving to the middleware package**,
  ...
  that.
- **`@markout-lang/bootstrap-kit`**, **`@markout-lang/std-kit`** — no TypeScript at
...
3. **Extract `@markout-lang/core`.** The largest step, and the one that unblocks
...
4. **Extract `@markout-lang/express`.** Small, once core is out. **Done** — four
...
6. **The site.** **Done.** [sites/site/](../../sites/site/) is one docroot with
   ...
   `@markout-lang/express` is for, including the ordering rule step 4 turned up:
